...
class MazeValidator:
    ...
    def validate_maze(self, lines: list) -> bool:
        ...
        for i, line in enumerate(lines):
            ...
            if not re.match(r'^[0-9A-Fa-f]+$', line):
                self.errors.append(f"Linha {i}: caracteres hexadecimais inválidos")
                return False
            
            if len(line) != width:
                self.errors.append(f"Linha {i}: largura inconsistente ({len(line)} vs {width})")
                return False
    ...
    def _validate_metadata(self, lines: list, maze_lines: list) -> bool:
        if len(lines) < 3:
            self.errors.append("Metadados incompletos (esperado: entrada, saída, caminho)")
            return False
